...
export default function Footer4(props: FooterProps = {}) {
	...
	useEffect(() => {
		...
		if (Object.keys(props).length > 0) {
			console.log("Footer4: Using provided props data");
			console.log("Footer4 received columns:", props.columns?.length);
			console.log("Footer4 app links:", props.showAppLinks, props.appLinks?.length);
			setData(props);
		} else {
			console.log("Footer4: Using local footer data");
			setData(footerData);
		}
	}, [props])
					...
					<div className="container-fluid bg-6">
						<div className="container position-relative z-2">
							...
							<div className="row text-center py-4 border-top border-white border-opacity-10">
								<span className="text-white opacity-50">{safeData.copyright}</span>
							</div>
						</div>
					</div>
				</div>
