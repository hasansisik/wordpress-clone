...
export default function Menu({ menuItems = [] }: MenuProps) {
	const [data, setData] = useState<any>(menuItems.length > 0 ? menuItems : menuData.mainMenuItems || [])
			...
			<div className="d-none d-lg-flex">
				<ul className="navbar-nav mx-auto gap-4 align-items-lg-center">
					{data?.map((item: any, index: number) => (
						<li key={index} className={`nav-item ${item?.dropdown ? `dropdown ${item?.dropdownType}` : ''}`}>
							<Link 
								className="nav-link fw-bold d-flex align-items-center" 
								href={item?.href || "#"} 
							...
							{item?.dropdown && item?.dropdownType === "menu-item-has-children" && (
						<div className="dropdown-menu fix">
							<ul className="list-unstyled">
										{item?.dropdownItems?.map((dropdownItem: any, i: number) => (
											<li key={i} className="position-relative z-1 border-bottom">
												<Link className="dropdown-item position-relative z-1 d-flex align-items-start" href={dropdownItem?.href || "#"}>
													{dropdownItem?.icon && getMenuIconByName(dropdownItem.icon)}
										<span className="ms-2">
														<span className="d-block fw-bold fs-6">{dropdownItem?.title || ""}</span>
														{dropdownItem?.subtitle && <span className="text-600 fs-7">{dropdownItem.subtitle}</span>}
										</span>
									</Link>
								</li>
										)) || []}
										...
										<div className="mega-menu-content">
											<div className="row">
													...
													<div className="mega-menu-list">
																	<p className="text-primary fw-bold mb-1 fs-7">{section?.title || ""}</p>
														<ul className="list-unstyled">
																		{section?.links?.map((link: any, linkIndex: number) => (
																			<li key={linkIndex}>
																				<Link className="dropdown-item position-relative z-1" href={link?.href || "#"}>
																					{link?.title || ""}
																				</Link>
																			</li>
																		)) || []}
						...
						<div className="mega-menu fix">
							<div className="d-flex flex-wrap gap-2 position-relative z-1">
										{item?.sections?.map((section: any, i: number) => (
											<Link key={i} href={section?.href || "#"} className="btn btn-md btn-filter mb-2 me-2 rounded-pill py-2 d-inline-flex">
												{section?.title || ""}
