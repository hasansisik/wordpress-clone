...
export default function Blog6({ previewData, selectedCategory }: Blog6Props) {
	...
	const { blogs, categories, loading: blogLoading, error } = useSelector((state: RootState) => state.blog)
		...
		if (selectedCategory) {
			const filtered = blogs.filter(blog => {
				if (Array.isArray(blog.category)) {
					return blog.category.some((cat: string) => 
				...
				<div className="container">
					<div className="row align-items-end ">
						<div className="col-12">
							<h4 className="ds-5 mt-3 mb-3" data-aos="fade-zoom-in" data-aos-delay={100} style={titleStyle}>
								{selectedCategory 
									? `Kategori: ${selectedCategory}` 
									: 'Tüm Blog Yazıları'}
							</h4>
							<span className="fs-5 fw-medium" data-aos="fade-zoom-in" data-aos-delay={200} style={subtitleStyle}>
								{selectedCategory 
					...
					<div className="row">
						{blogPosts.length === 0 ? (
							<div className="col-12 text-center py-5">
								<p className="text-muted">Bu kategoride henüz blog yazısı bulunmamaktadır.</p>
							</div>
						) : (
											...
											<div className="d-flex flex-wrap gap-1 mt-3">
												{Array.isArray(post.category) ? (
													post.category.map((cat: string, idx: number) => (
														...
														>
															<span className="tag-spacing fs-7 fw-bold text-uppercase">{cat}</span>
														</button>
													))
												) : (
													<button
													...
													>
														<span className="tag-spacing fs-7 fw-bold text-uppercase">{post.category}</span>
													</button>
												)}
											</div>
